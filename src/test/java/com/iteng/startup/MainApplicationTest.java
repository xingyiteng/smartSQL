...
public class MainApplicationTest {
    ...
    void insertTest() {
        User user = new User();
        user.setUserAccount("1475208984@qq.com");
        user.setUserPassword(DigestUtils.md5DigestAsHex((UserConstant.USER_SALT + "123456").getBytes()));
        user.setUsername("iteng");
        user.setGender(0);
        user.setPhone("15188991234");
        user.setAvatarUrl("http://himg.bdimg.com/sys/portrait/item/public.1.711688ef.AxcLvbRPlaTaEqO_49mhtg.jpg");
        user.setUserRole(1);
    ...
    void sparkTest() {
        ...
        messages.add(SparkMessage.userContent("CREATE TABLE `file_catalog` (\n" +
                "  `catalog_id` int NOT NULL AUTO_INCREMENT COMMENT '目录编号',\n" +
                "  `parent_catalog_id` int DEFAULT NULL COMMENT '上级目录编号',\n" +
                "  `catalog_name` varchar(255) CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci DEFAULT NULL COMMENT '目录名称',\n" +
                "  `level` int DEFAULT NULL COMMENT '目录级别',\n" +
                "  `sort_number` int DEFAULT NULL COMMENT '排序号',\n" +
                "  PRIMARY KEY (`catalog_id`) USING BTREE\n" +
                ") ENGINE=InnoDB AUTO_INCREMENT=530 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC;诉求：删除名称为idx_level的索引"));
        // 构造请求
        ...
        try {
            ...
            String s = extractSqlQuery(chatResponse.getContent());
            System.out.println(s);
            if(!isValidSqlStatement(s)){
                return;
            }
            String s1 = executeSql(s);
    ...
    public String executeSql(String sql) {
        String url = "jdbc:mysql://localhost:3306/devfzjc";
        String user = "root";
    ...
    @Test
    void explainTest() {
        String s = executeSql("EXPLAIN SELECT * FROM file_catalog WHERE level = 1;");
        ...
        // 消息列表，可以在此列表添加历史对话记录
        List<SparkMessage> messages = new ArrayList<>();
        messages.add(SparkMessage.systemContent("接下来每次给你一个SQL操作语句，以及MySQL数据库通过EXPLAIN关键字分析之后的结果，格式如下：\n" +
                "{{SQL}}\n" +
                "{{EXPLAIN分析结果}}\n" +
                ...
                "结论：{{根据以上属性和SQL语句，总结索引命中情况、是否使用全表扫描。如果没有命中索引，请给出具体优化操作，如：新建索引或优化查询条件等等，并给出SQL操作语句}}"));
        messages.add(SparkMessage.userContent("SELECT * FROM file_catalog WHERE level = 1;"+s));
        // 构造请求
        SparkRequest sparkRequest = SparkRequest.builder()
                // 消息列表
                .messages(messages)
